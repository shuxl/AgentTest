...
def create_ivfflat_index(client: PgVectorClient, table_name: str, index_name: str,
...
def test_query_performance(client: PgVectorClient, table_name: str, 
    ...
    Args:
        ...
        table_name: 表名
        dimension: 向量维度
        probes: IVFFlat 查询参数 probes（探测的聚类数量）
        num_queries: 测试查询数量
        k: 返回 Top-K 结果
    ...
    Returns:
        dict: 查询性能统计信息
    """
    print(f"\n测试查询性能 (probes={probes}, k={k})")
    
    # 设置查询参数
    client.execute_update(f"SET ivfflat.probes = {probes};")
...
def main():
    parser = argparse.ArgumentParser(description='测试 IVFFlat 索引性能')
    parser.add_argument('--table-name', type=str, default=TEST_TABLE_NAME,
                       help=f'表名（默认：{TEST_TABLE_NAME}）')
    parser.add_argument('--index-name', type=str, default='idx_items_embedding_ivfflat',
                       help='索引名（默认：idx_items_embedding_ivfflat）')
    parser.add_argument('--lists', type=int, default=None,
                       help='IVFFlat 参数 lists（聚类中心数量，默认：rows/1000）')
    ...
    if count < 1000:
        print("⚠️  警告：IVFFlat 索引建议至少 1000 条数据，当前只有 {count} 条")
    
    # 确定 lists 参数
    if args.lists is None:
        # 默认 lists = rows / 1000
        args.lists = max(10, count // 1000)
        print(f"\n使用默认 lists 参数: {args.lists} (rows/{count//args.lists if args.lists > 0 else 1})")
    
    # 创建索引
    index_info = None
    if not args.skip_create:
        index_info = create_ivfflat_index(
            client, args.table_name, args.index_name,
            dimension, args.distance_op, args.lists
        )
    ...
    if not args.skip_query:
        query_stats = test_query_performance(
            ...
            args.probes, args.num_queries, args.k
        )
