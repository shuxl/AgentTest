...
- `domain/models/schemas/` - SQLAlchemy 数据模型定义
- `domain/models/base.py` - Base 类和数据库引擎管理
- `alembic/` - Alembic 迁移配置
- `docs/数据库使用指南.md` - 数据库使用文档

**注意**：数据模型模块已从 `utils/db/` 迁移到 `domain/models/`，测试文件仍使用向后兼容层 `utils.db`，建议新代码使用 `domain.models`。
