...
from domain.router import RouterState, IntentResult, router_node, route_decision, clarify_intent_node
from domain.router.tools import identify_intent, clarify_intent
...
async def test_identify_intent_tool():
    ...
    # 测试明确的意图
    # 注意：根据当前设计，意图类型为：blood_pressure, appointment, diagnosis, unclear
    # diagnosis 是主意图类型，有多个子类型（如 internal_medicine_diagnosis 等）
    test_cases = [
        ("我想记录血压", "blood_pressure"),
        ("查询我的血压记录", "blood_pressure"),
        ("我想预约复诊", "appointment"),
        ("查询我的预约", "appointment"),
        ("查询患者病历", "diagnosis"),  # 已更新：从 doctor_assistant 改为 diagnosis
        ("帮我诊断这个患者", "diagnosis"),  # 新增：明确的诊断意图
        ("你好", "unclear"),
    ]
    ...
    for query, expected_intent in test_cases:
        try:
            ...
            # 注意：diagnosis 是主意图类型，可能返回 diagnosis 或具体的子类型（如 internal_medicine_diagnosis）
            valid_intent_types = ["blood_pressure", "appointment", "diagnosis", "internal_medicine_diagnosis", 
                                 "surgery_diagnosis", "pediatrics_diagnosis", "gynecology_diagnosis",
                                 "cardiology_diagnosis", "general_diagnosis", "unclear"]
            assert intent_type in valid_intent_types
            assert 0.0 <= result.confidence <= 1.0
            
            # 3. 验证识别出的意图是否等于期望的意图（精确验证）
            # 对于 diagnosis 类型，可能返回 diagnosis 或具体的子类型（如 internal_medicine_diagnosis）
            if intent_type == expected_intent:
                intent_match = True
            elif expected_intent == "diagnosis":
                # 如果期望是 diagnosis，实际返回的是 diagnosis 或其子类型，也认为是匹配的
                diagnosis_types = ["diagnosis", "internal_medicine_diagnosis", "surgery_diagnosis", 
                                  "pediatrics_diagnosis", "gynecology_diagnosis", "cardiology_diagnosis",
                                  "general_diagnosis"]
                intent_match = intent_type in diagnosis_types
            else:
            ...
            # 5. 验证路由决策：route_decision 直接返回 current_intent
            # 所以 route 应该等于 intent_type
            route_match = (route == intent_type)
            
            if intent_match and route_match:
                print(f"  ✓ '{query}' -> intent={intent_type} (置信度: {result.confidence:.2f}), route={route}")
                passed += 1
            else:
                print(f"  ✗ '{query}' -> intent={intent_type} (期望: {expected_intent}), route={route} (期望: {intent_type})")
